...
export class Layout {
	...
	async getNodeTree(element, parent = {}, index = 0, siblings = [], source) {
		const computedStyle = Object.assign({}, this.getComputedStyle(element, parent, index));
		const attributes = await this.getAttributes(element)
		const node = {
			id: uuid++,
			parent,
			computedStyle,
	...
	getComputedStyle(element, parent = {}, index = 0) {
		...
		for (const value of Object.keys(node)) {
			...
			if (/^(box)?shadow$/i.test(value)) {
				const shadows = item.split(' ').map(v => /^\d/.test(v) ? toPx(v) : v)
			...
			if (/^border(?!radius)/i.test(value)) {
				...
				const v = item.split(' ').map(v => /^\d/.test(v) ? toPx(v) : v)
			...
			if(/padding|margin|radius/i.test(value)) {
				const isRadius = value.includes('adius')
				const prefix = isRadius ? 'borderRadius' : value.match(/[a-z]+/)[0]
				const pre = [0,0,0,0].map((item, i) => isRadius ? ['borderTopLeftRadius', 'borderTopRightRadius', 'borderBottomRightRadius', 'borderBottomLeftRadius'][i] : [prefix + 'Top', prefix + 'Right', prefix + 'Bottom', prefix + 'Left'][i])
				if(value === 'padding' || value === 'margin' || value === 'radius' || value === 'borderRadius') {
					const v = item?.split(' ').map((item) => /^\d/.test(item) && toPx(item, node['width']), []) || [0];
					const type = isRadius ? 'borderRadius' : value;
					if(v.length == 1) {
						style[type] = v[0]
					} else {
						const [t, r, b, l] = v
	...
	getLayoutBox(element, parent = {}, index = 0, siblings = [], source = {}) {
		const box = {}
		const { name, computedStyle: cstyle, layoutBox, attributes } = element || {}
		if(!name) return box
		const { ctx } = this
		...
		if(name === 'text') {
			...
			const width = lineIndex > 1 ? pbox.width : remain + (box?.offsetLeft || 0);
			if(!islineBlock) {
				box.offsetRight = (x || 0) + box.offsetLeft + (w || (isblock ? pbox.width : remain)) + pr + mr;
			}
