这个是旧版本，它与新版本l-painter-new的区别：
一、旧版本的l-painter在小程序里的中划线line-through定位不对
二、新版本l-painter-new画图的窗口要fixed在视窗外，不能放在u-popup里

备注：
由于当其中任意一个图片的地址错误时，会导致整个海报都会报错而画不出来(暂时只解决了H5的问题，小程序端现在未找到解决方案)，所以本系统修改部分如下：
utils.js改动如下(一、加入图片错误时的默认图片；二、将reject删掉):  
image.onerror = (error) => {
