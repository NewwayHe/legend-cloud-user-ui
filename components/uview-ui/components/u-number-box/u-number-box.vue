...
<!--    /*
    *   变更：(2021.2.25)
    *       一、加入一个oldValue变量并修改onBlur方法，让如果删除掉input里所有的数字时，离后焦点后，让input框上的值变回原来未变化前的值
    * 
    *   用法:
    */ -->
	<view class="u-numberbox" :style="{'--u-icon-disabled': type==1?'':iconDisabledBg}">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 输入框获得焦点事件
			onFocus(e) {
                this.oldValue = Number(e.detail.value)
				this.$emit('focus');
			},
...
<style lang="scss" scoped>
	...
	.u-icon-disabled {
		...
		background: var(--u-icon-disabled) !important;
	}
