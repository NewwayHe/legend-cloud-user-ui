<template>
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			<!--图片-->
			<view v-if="n.name=='img'" :class="'_img '+n.attrs.class" :style="n.attrs.style +';vertical-align: bottom;'" :data-attrs="n.attrs" @tap="imgtap">
				...
				if (jump) {
					...
					if (attrs.href) {
						if (attrs.href[0] == '#') {
							...
							// #endif
							// #ifndef APP-PLUS
                            uni.navigateTo({ url: `/pages/webview/webview?url=${attrs.href}` })
