<template>
    <view class="w750" :style="{ '--themescolor': themes.color }">
        <!-- /头部 -->
		<view class="position-fixed top-0 w-100vw h-100vh text-center" style="background-color: #019fe8 ;" v-if="!isWeixin">
			<view class=" mt-15">
				<u-icon name="zhifubao-circle-fill" color="#fff" :size="160"></u-icon>
				<view class="text-fff font-40 letter-sp4 mt-3">我有我的方式</view>
			</view>
		</view>
		<template v-else>
			<view class="J-weixin-tip-img weixin-tip-img" :class="isApple ? 'iphone' : 'android'"></view>
		</template>
		<view class="pay" v-html="payData" v-show="payData"></view>
    </view>
</template>
